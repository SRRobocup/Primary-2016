...
void twoEye()
{
	...
	else if(getIncline() < -deg && (lastState != RIGHTBLACK || lastState != LEFTBLACK))
	{
		...
		if (lastIncline == 1)
		{
			writeDebugStreamLine("SPEED BUMP");
			while(!seeLine())
				arrayPID();
			lastIncline = -1;
			return;
	...
	if(getDigit(state,100) == 1)
		state1 |= 1<<2;

	if(getDigit(state,10) == 1)
		state1 |= 1<<1;

	if(getDigit(state,1) == 1)
		state1 |= 1;

	for (int i=ZIGLENGTH-1; i>=1; i--)
{	zig[i]=zig[i-1];
//writeDebugStreamLine("zig:%d %d", i, zig[i]);
}
	zig[0]=state1;

	for (int i=1; i<=ZIGLENGTH-1; i++)
	{
		if (((zig[i]^zig[i-1]) | 3) > 0)
			ftt++;

		if (((zig[i]^zig[i-1]) | 5) > 0)
			tft++;
	//writeDebugStreamLine("xor:%d %d", i, zig[i]^zig[i-1]);
	}

	if (ftt >= (20) && tft >= (20))
	{
		 println (5,"Zigzag detected");
	//	playSound(soundBeepBeep);
	//	wait1Msec(100);

	writeDebugStreamLine("ZIGZAG");
//		writeDebugStreamLine(" %d %d", ftt, tft);
	//	stopMotors();
	//	playSound(soundDownwardTones);
//		wait1Msec(100);
  }
