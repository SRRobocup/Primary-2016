...
task main()
{
	...
	MSLSAinit(splitter);
	generateLight(0x52,19,55,2000,&Meye);
	RMotor = motorC;
	LMotor = motorA;
	long delete;
	...
	display(2, "%.1f %.1f", determineThresh(black[0],green[0][0]), determineThresh(white[0],green[0][1]));
	display(3, "%.2f",min[0]);
	display(4, "%d %d %d",black[1]+5, (black[1] + white[1])/2, (silver + white[1])/2);
	display(5, "%.1f %.1f", determineThresh(black[2],green[1][0]), determineThresh(white[2],green[1][1]));
	display(6, "%.2f",min[1]);
	writeDebugStreamLine("{%.1f,%.1f,%.1f,%.2f,%d,%d,%d,%.1f,%.1f,%.1f,%.2f}",determineThresh(black[0],green[0][0]), determineThresh(white[0],green[0][1]),(float)(black[0] + white[0])/2,min[0],black[1]+5, (black[1] + white[1])/2,silver-5,
																																			  determineThresh(black[2],green[1][0]), determineThresh(white[2],green[1][1]),(float)(black[2] + white[2])/2, min[1]);
	while(true){}
}
