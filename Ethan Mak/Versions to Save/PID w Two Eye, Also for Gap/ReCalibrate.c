...
task main()
{
	...
	display(6, "%.2f",min[1]);
	writeDebugStreamLine("{%.1f,%.1f,%.1f,%.2f,%d,%d,%d,%.1f,%.1f,%.1f,%.2f}",(float)(black[0] + green[0][0])/2, (float)(green[0][1] + white[0])/2,(float)(black[0] + white[0])/2,min[0],black[1]+5, (black[1] + white[1])/2,(silver + white[1])/2,
																																			 (float)(black[2] + green[1][0])/2, (float)(green[1][1] + white[2])/2,(float)(black[2] + white[2])/2, min[1]);
	while(true){}
}
